import re
from tokenizer import tokenize, feature_set, no_of_items


# To calculate the basic probability of a word for a category
def calc_prob(word, category):
    if word not in feature_set or word not in tokenize[category]:
        return 0

    return float(tokenize[category][word]) / no_of_items[category]
